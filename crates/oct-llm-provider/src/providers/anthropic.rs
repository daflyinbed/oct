    ...
    fn build_headers(&self) -> Result<HeaderMap, ModelError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&api_key).map_err(|err| {
                ModelError::transport(format!("invalid anthropic api key header: {err}"))
            })?,
        );
    ...
    for part in &message.parts {
        content.push(match part {
            ...
            ContentPart::Reasoning(text) => {
                AnthropicContentBlock::Thinking(AnthropicThinkingBlock {
                    thinking: text.clone(),
                    signature: None,
                })
            }
...
fn map_anthropic_tool_choice(choice: &crate::core::ToolChoice) -> AnthropicToolChoice {
    match choice {
        crate::core::ToolChoice::Auto => AnthropicToolChoice::Auto {
            disable_parallel_tool_use: None,
        },
        crate::core::ToolChoice::None => AnthropicToolChoice::None,
        crate::core::ToolChoice::Required => AnthropicToolChoice::Any {
            disable_parallel_tool_use: None,
        },
        crate::core::ToolChoice::Named(name) => AnthropicToolChoice::Tool {
            name: name.clone(),
            disable_parallel_tool_use: None,
        },
    }
...
pub fn map_anthropic_response(payload: AnthropicResponse) -> Result<ChatResponse, ModelError> {
    ...
    if let Some(container) = &payload.container {
        provider_metadata.insert(
            "container".to_string(),
            serde_json::to_value(container).unwrap_or(Value::Null),
        );
    }
...
fn parse_anthropic_content_block(block: AnthropicContentBlock) -> Result<ContentPart, ModelError> {
    match block {
        ...
        AnthropicContentBlock::ToolUse(tool_block) => {
            Ok(ContentPart::ToolCall(crate::core::ToolCall {
                id: tool_block.id,
                name: tool_block.name,
                arguments: serde_json::to_string(&tool_block.input).unwrap_or_default(),
            }))
        }
        AnthropicContentBlock::ToolResult(result_block) => {
            Ok(ContentPart::ToolResult(crate::core::ToolResult {
        ...
        AnthropicContentBlock::Thinking(thinking_block) => {
            Ok(ContentPart::Reasoning(thinking_block.thinking))
        }
        AnthropicContentBlock::RedactedThinking(redacted_block) => Ok(ContentPart::Reasoning(
            format!("[redacted: {}]", redacted_block.data),
        )),
        AnthropicContentBlock::Image(_) => {
            Err(ModelError::provider("unexpected image block in response"))
        }
    }
}
...
/// events are ever decoded (in the stream loop above): a multibyte UTF-8
...
fn normalize_anthropic_stream_event_with_state(
    ...
    match event {
        ...
        AnthropicStreamEvent::ContentBlockStart {
            ...
            AnthropicContentBlock::Text(text_block) if !text_block.text.is_empty() => {
        ...
        AnthropicStreamEvent::ContentBlockStop { index } => {
            if let Some(acc) = tool_state.remove(&index)
                && !acc.emitted
                && (acc.name.is_some() || !acc.arguments.is_empty())
            {
                events.push(StreamEvent::ToolCall(crate::core::ToolCall {
                    id: acc.id.unwrap_or_else(|| format!("tool_{}", index)),
                    name: acc.name.unwrap_or_default(),
...
mod sse_tests {
    ...
    #[test]
    fn anthropic_sse_survives_multibyte_splits_across_chunks() {
        ...
        let expected = parse_anthropic_sse_transcript(transcript).unwrap();
        assert_eq!(
            expected,
            vec![
        ...
        for size in 1..transcript.len() {
            ...
            for chunk in transcript.as_bytes().chunks(size) {
                ...
                while let Some(event) = take_sse_event(&mut buffer) {
                    ...
                    got.extend(
                        parse_anthropic_sse_event_with_state(
                            &event,
                            &mut tool_state,
                            &mut stop_reason,
                        )
            ...
            for remainder in flush_sse_buffer(&mut buffer) {
                let event = decode_sse_event(remainder)
                    .unwrap_or_else(|err| panic!("chunk size {size}: {err}"));
                got.extend(
                    parse_anthropic_sse_event_with_state(&event, &mut tool_state, &mut stop_reason)
                        .unwrap_or_else(|err| panic!("chunk size {size}: {err}")),
                );
            }
