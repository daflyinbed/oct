...
#[cfg(test)]
fn parse_openai_sse_event(event: &str) -> Result<Vec<StreamEvent>, ModelError> {
...
mod tests {
    ...
    use crate::core::{ContentPart, FinishReason, GenerateOptions, Message, Role, ToolChoice, ToolSpec};
    ...
    use super::{
        ...
        normalize_openai_stream_chunk, parse_openai_sse_event, OpenAiCompatibleChatModel,
    ...
    fn maps_chat_request_to_openai_wire_request() {
        let model = OpenAiCompatibleChatModel::new(OpenAiCompatibleConfig {
            ...
            model_info: ModelInfo::new("openai", "gpt-4.1").with_capabilities(ModelCapabilities {
                streaming: true,
                native_tools: true,
                vision: true,
                json_mode: true,
        ...
        assert_eq!(map_openai_finish_reason(Some("tool_calls")), FinishReason::ToolCalls);
    }
    ...
    fn normalizes_openai_generate_response() {
        ...
        assert_eq!(response.usage.as_ref().and_then(|u| u.total_tokens), Some(14));
    ...
    fn normalizes_openai_stream_chunks() {
        ...
        assert!(matches!(&events[1], crate::core::StreamEvent::TextDelta(text) if text == "hel"));
        assert!(matches!(&events[2], crate::core::StreamEvent::ToolCallDelta { .. }));
        assert!(matches!(&events[3], crate::core::StreamEvent::ToolCall(call) if call.name == "lookup"));
        assert!(matches!(&events[4], crate::core::StreamEvent::Finish(FinishReason::ToolCalls)));
    }

    #[test]
    fn parses_openai_sse_done_event() {
        let events = parse_openai_sse_event("data: [DONE]").unwrap();
        assert_eq!(events, vec![crate::core::StreamEvent::Finish(FinishReason::Stop)]);
    }
    ...
    fn emits_final_openai_tool_call_when_done() {
        ...
        assert!(matches!(&events[0], crate::core::StreamEvent::ToolCallDelta { .. }));
        assert!(matches!(&events[1], crate::core::StreamEvent::ToolCall(call) if call.name == "lookup"));
    }
}
